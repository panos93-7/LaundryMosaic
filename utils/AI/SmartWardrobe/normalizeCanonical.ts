// utils/SmartWardrobe/normalizeCanonical.ts
...
export function normalizeCanonical(raw: any): WardrobeCanonical {
  ...
  const mapColor = (color: string) => {
    const c = norm(color);
    if (c.includes("light grey") || c.includes("light gray")) return "light_gray";
    if (c.includes("grey") || c.includes("gray")) return "gray";
  ...
  const mapFabric = (fabric: string) => {
    const f = norm(fabric);
    if (f.includes("cotton") && f.includes("blend")) return "cotton_blend";
  ...
  const mapCareSymbol = (symbol: string) => {
    const s = norm(symbol);
    if (s.includes("30")) return "WashAt30";
  ...
  return {
    ...
    careSymbols: (raw.careSymbols ?? []).map(mapCareSymbol).sort(),
